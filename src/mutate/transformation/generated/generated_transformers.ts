...
function getValueByPath(obj: any, path: (string | number)[]): any {
  ...
  for (const key of path) {
    ...
    current = Array.isArray(current) && typeof key === "number"
      ? current[key]
      : (typeof key === "string"
...
function setValueByPath(
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (typeof key === "string") currentKey = key;
      ...
      console.error(`setValueByPath: Invalid key ${String(key)}`);
      return false;
    }
  ...
  if (
    ...
    console.error(`setValueByPath: Invalid final key ${String(finalKey)}`);
    return false;
  }
}
...
async function applyRules(
  ...
  console.log(
    `Applying ${direction} transformation rules from ${ruleFileName}...`,
  );
  ...
  for (const rule of rules) {
    console.log(
      `  [${direction}] Executing rule: ${rule.name} (${rule.transformer_function})`,
    );
    const transformerFn =
      (transformers as Record<string, Function>)[rule.transformer_function];
    if (!transformerFn) {
      console.warn(
        `  [${direction}] Unknown transformer function: ${rule.transformer_function}. Skipping.`,
      );
      continue;
    }

    const sourceValue = rule.source_path
      ? getValueByPath(sourceConfig, rule.source_path)
      : undefined;
    const targetValue = rule.target_path
      ? getValueByPath(transformedConfig, rule.target_path)
      : undefined;
    ...
    try {
      ...
      if (
        result && typeof result === "object" && result.newValue !== undefined &&
        rule.target_path
      ) {
        const success = setValueByPath(
          transformedConfig,
          rule.target_path,
          result.newValue,
        );
        if (!success) {
          console.error(
            `  [${direction}] Failed to set value for rule '${rule.name}' at path ${
              JSON.stringify(rule.target_path)
            }.`,
          );
        }
        ...
        if (result.changed) {
          console.log(
            `  [${direction}] Transformer for rule '${rule.name}' indicated changes.`,
          );
        }
      }
    } catch (error) {
      console.error(
        `  [${direction}] Error executing rule '${rule.name}':`,
        error,
      );
    }
  }
  ...
  if (overallChanged) {
    console.log(
      `${direction} transformations from ${ruleFileName} resulted in overall changes.`,
    );
  } else {
    console.log(
      `${direction} transformations from ${ruleFileName} resulted in no changes.`,
    );
  }
...
export async function applyMiseToTrunkRules(
  ...
  const rules = [{
    ...
    "description":
      "Synchronizes lint.enabled in trunk.yaml based on parsed linter list from mise.toml",
    "target_path": ["lint", "enabled"],
    "source_path": ["_", "devtools", "trunk", "enabled_linters"],
    "transformer_function": "syncLinters",
    "priority": 10,
    "context": {
      "linterFormatOptions": { "git-diff-check": { "noVersionSuffix": true } },
    },
  }, {
    "name": "SyncRuntimeDefinitions",
    "description":
      "Synchronizes runtimes.definitions in trunk.yaml with tools in mise.toml",
    "target_path": ["runtimes", "definitions"],
    "source_path": ["tools"],
    "transformer_function": "syncRuntimeDefinitions",
    "priority": 20,
    "context": { "runtimeMapping": { "nodejs": "node" } },
  }, {
    "name": "GenerateEnabledRuntimes",
    "description":
      "Generates runtimes.enabled list based on runtimes.definitions",
    "target_path": ["runtimes", "enabled"],
    "source_path": ["runtimes", "definitions"],
    "transformer_function": "generateEnabledListFromDefinitions",
    "priority": 25,
  }, {
    "name": "UpdateTrunkVersionField",
    "description":
      "Updates the top-level 'version' field in trunk.yaml based on mise.toml tools.trunk",
    "target_path": ["version"],
    "source_path": ["tools", "trunk"],
    "transformer_function": "updateTrunkVersion",
    "priority": 30,
    "context": { "ignoreSchemaVersion": "0.1" },
  }] as TransformRule[];
  return applyRules(
    "Mise->Trunk",
    targetConfig,
...
export async function applyTrunkToMiseRules(
  ...
  console.log(
    "Skipping Trunk -> Mise transformations: No rules defined in trunk_to_mise.toml.",
  );
