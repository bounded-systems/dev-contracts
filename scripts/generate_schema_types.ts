...
export interface MiseConfig {
  ...
  settings?: {
    ...
    const typesFilePath = path.join(this.typesDir, "linter-types.ts");
    await Deno.writeTextFile(typesFilePath, typesContent);
    console.log(`Generated linter types at ${typesFilePath}`);
  }
  ...
  async validateAndGenerate(): Promise<boolean> {
    try {
      ...
      // Validate trunk.yaml
      console.log("Validating .trunk/trunk.yaml...");
      ...
      console.log(`.trunk/trunk.yaml: ${trunkValidation.valid ? "✅ VALID" : "❌ INVALID"}`);
  ...
  async transformTrunkConfig(): Promise<boolean> {
    try {
      ...
      if (miseConfig.tools) {
        ...
        // Map of mise tool names to trunk runtime names
        ...
        for (const [tool, version] of Object.entries(miseConfig.tools)) {
          if (tool in toolMapping && typeof version === "string") {
            const runtimeName = toolMapping[tool];
            const runtimeVersion = version;
            const runtimeString = `${runtimeName}@${runtimeVersion}`;
      ...
      if (miseConfig.settings?.devtools?.trunk?.enabled_linters) {
        ...
        for (const linter of miseConfig.settings.devtools.trunk.enabled_linters) {
          ...
          const existingIndex = trunkConfig.lint.enabled.findIndex(l => {
            if (typeof l === "string") {
              return l.startsWith(`${linterName}@`);
            } else if (typeof l === "object" && l.name) {
              return String(l.name).startsWith(`${linterName}@`);
            }
            return false;
          });
