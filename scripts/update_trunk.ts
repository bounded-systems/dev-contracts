...
export async function updateTrunkConfig(
  ...
  try {
    ...
    if (miseConfig.tools || miseConfig.settings?.trunk) {
      ...
      // --- Populate Linters from settings.trunk.linters ---
      console.log("Populating linters from mise.toml settings.trunk.linters...");
      const trunkLintersSettings = miseConfig.settings?.trunk?.linters;
      if (trunkLintersSettings && typeof trunkLintersSettings === "object") {
        // Ensure lint and enabled array exist (already done at init)
        // trunkConfig.lint = trunkConfig.lint ?? { enabled: [] };
        // trunkConfig.lint.enabled = trunkConfig.lint.enabled ?? [];

        for (const [linterName, isEnabled] of Object.entries(trunkLintersSettings)) {
          if (!isEnabled) {
            console.log(`Linter '${linterName}' is present in settings but not enabled.`);
            continue;
          }

          // Find the actual version defined for the linter tool in [tools]
          const linterVersion = miseConfig.tools?.[linterName];

          if (linterVersion === undefined || linterVersion === null) {
            console.warn(
              `Skipping linter "${linterName}" because it is enabled in [settings.trunk.linters] but not defined in [tools].`
            );
            continue;
          }

          if (typeof linterVersion !== "string") {
            console.warn(
              `Skipping linter "${linterName}" because its version in [tools] (${linterVersion}) is not a string.`
            );
            continue;
          }
          ...
          if (linterName === "git-diff-check") {
            // git-diff-check uses "enabled" in mise [tools], translate to SYSTEM for trunk
            if (linterVersion === "enabled") {
              linterEntry = `${linterName}@SYSTEM`;
              console.log(`Adding system linter: ${linterEntry}`);
            } else {
              console.warn(
                `Skipping git-diff-check: Enabled in settings, but expected version 'enabled' not found in [tools] (found: '${linterVersion}')`
              );
              continue;
            }
          } else if (linterVersion === "enabled" || linterVersion === "SYSTEM") {
            // This case should ideally not happen if versions are properly specified in [tools],
            // but handle it just in case.
            console.warn(
              `Linter "${linterName}" is enabled in settings, but its version in [tools] is "${linterVersion}". Adding to Trunk without a specific version.`
            );
            linterEntry = linterName;
          } else {
            // Standard case: use the version defined for the tool (e.g., eslint = "9.24.0")
            linterEntry = `${linterName}@${linterVersion}`;
        ...
        console.log("No settings.trunk.linters found in mise.toml or it's not an object.");
      }
      // --- End of Linter Population Logic ---
